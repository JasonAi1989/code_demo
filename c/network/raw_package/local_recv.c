...
int local_recv(char* interface, int protocol)
{
	int sock_raw_fd = socket(PF_PACKET, SOCK_RAW, htons(protocol));
	if(sock_raw_fd < 0){
		perror("socket");
		return -1;
	}
    ...
    if(setsockopt(sock_raw_fd, SOL_SOCKET, SO_BINDTODEVICE, interface, strlen(interface)+1) == -1){
        perror("setsockopt");
        close(sock_raw_fd);
        exit(-1);
    }
...
int parse_packet(unsigned char *buf_p, int data_size)
{
    ...
    sprintf(filename, "recv_%d.txt", counter);
